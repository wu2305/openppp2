...
#define PPP_APPLICATION_VERSION ("1.0.0.24061") /* 10.0.0.240205 */
#endif
...
PppApplication::PppApplication() noexcept
{
    ...
    {
        COORD cSize = { 120, 41 }; /* ppp::win32::Win32Native::IsWindows11OrLaterVersion(); */
...
bool PppApplication::PrintEnvironmentInformation() noexcept
{
    ...
    if (NULL != server)
    {
        ...
        if (auto managed_server = server->GetManagedServer(); NULL != managed_server)
        {
...
bool PppApplication::PreparedLoopbackEnvironment(bool client_or_server, const std::shared_ptr<NetworkInterface>& network_interface) noexcept
{
    ...
    if (client_or_server)
    {
        ...
        do
        {
            ...
            ethernet = ppp::make_shared_object<VEthernetNetworkSwitcher>(context, network_interface->Lwip, network_interface->VNet, configuration);
            if (NULL == ethernet)
            {
                break;
            }
...
void PppApplication::PrintHelpInformation() noexcept
{
    ...
    messages += "        ./" + execution_file_name + " --pull-iplist \r\n";
    ...
    fprintf(stdout, messages.data(), PPP_APPLICATION_VERSION, execution_file_name.data(), BOOST_BEAST_VERSION_STRING);
        ...
        else
        {
            context->post(
                [context]() noexcept
                {
                    ...
                    {
                        auto timeout = Timer::Timeout(context, 1000, fx);
                        return NULL != timeout;
                    }
                });
            return true;
