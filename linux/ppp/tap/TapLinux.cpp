...
namespace ppp {
    namespace tap {
        ...
        static bool SetRouteToLinux(const ppp::string& ifrName, UInt32 address, int prefix, UInt32 gw, bool action_add_or_delete) noexcept {
            int len = 0;
        ...
        bool TapLinux::AddRoute(const ppp::string& ifrName, UInt32 address, int prefix, UInt32 gw) noexcept {
            ...
            if (ifc_ctl_sock_compatible_route) {
                return SetRouteToLinux(ifrName, address, prefix, gw, true);
            }
            else {
        ...
        bool TapLinux::DeleteRoute(const ppp::string& ifrName, UInt32 address, int prefix, UInt32 gw) noexcept {
            ...
            if (ifc_ctl_sock_compatible_route) {
                return SetRouteToLinux(ifrName, address, prefix, gw, false);
            }
            else {
                struct in_addr in_dst;
                struct in_addr in_gw;
        ...
        */
        ...
        bool TapLinux::Output(const std::shared_ptr<Byte>& packet, int packet_size) noexcept {
            return WritePacketToKernelNio(this, disposed_, packet.get(), packet_size);
        }

        bool TapLinux::Output(const void* packet, int packet_size) noexcept {
            return WritePacketToKernelNio(this, disposed_, packet, packet_size);
        }
        ...
        bool TapLinux::SetNetifUp(bool up) noexcept {
            ...
            if (up) {
                ...
                if (ioctl(ifc_ctl_sock.sock_v4, SIOCSIFMTU, &ifr) < 0) {
                    return false;
                }
            }
