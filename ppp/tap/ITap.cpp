...
namespace ppp
{
    namespace tap
    {
        ...
        std::shared_ptr<ITap> ITap::From(const std::shared_ptr<boost::asio::io_context>& context, const ppp::string& id, void* tun, uint32_t address, uint32_t gw, uint32_t mask, bool promisc, bool hosted_network) noexcept
        {
            ...
            if (TapLinux::GetInterfaceName(static_cast<int>(reinterpret_cast<std::intptr_t>(tun)), interface_name)) 
            {
