...
namespace stl {
    ...
    inline TOUT                                                             transform(const TIN& s) noexcept {
        return TOUT(s.data(), s.size());
    }

    template <typename TString>
    inline TString                                                          to_string(float num) noexcept {
        char buf[536];
        return snprintf(buf, sizeof(buf), "%f", num) > 0 ? buf : "";
    }

    template <typename TString>
    inline TString                                                          to_string(double num) noexcept {
        char buf[536];
        return snprintf(buf, sizeof(buf), "%lf", num) > 0 ? buf : "";
    }

    template <typename TString>
    inline TString                                                          to_string(long double num) noexcept {
        char buf[536];
        return snprintf(buf, sizeof(buf), "%Lf", num) > 0 ? buf : "";
    }
...
namespace ppp {
    ...
    template<typename _Ty> 
    inline _Ty                                                              ATrim(const _Ty& s) noexcept {
        if (s.empty()) {
            return s;
        }
    ...
    template<typename _Ty>
    inline int                                                              Split(const _Ty& str, ppp::vector<_Ty>& tokens, const _Ty& delimiters) noexcept {
        if (str.empty()) {
            return 0;
        }
        elif(delimiters.empty()) {
    ...
    template<typename T>
    inline T                                                                exchangeof(T& location, const T& value) noexcept {
        T r = location;
        location = value;
        return r;
    }
    ...
    template<typename T>
    inline constexpr T&                                                     nullof() noexcept { return *(T*)NULL;}
    ...
    template<typename T, typename... A>
    inline std::shared_ptr<void*>                                           make_shared_void_pointer(A&&... args) noexcept {
        static_assert(sizeof(T) > 0, "can't make pointer to incomplete type");
