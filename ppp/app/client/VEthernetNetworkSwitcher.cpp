    ...
    namespace app {
        namespace client {
            ...
            bool VEthernetNetworkSwitcher::Constructor(const std::shared_ptr<ITap>& tap) noexcept {
                ...
                if (ppp::string cidrs = std::move(bypass_ip_list_); cidrs.size() > 0) {
                    if (RouteInformationTablePtr rib = make_shared_object<RouteInformationTable>(); NULL != rib) {
                        ...
                        if (rib->AddAllRoutes(cidrs, tap->IPAddress)) {
                            rib_ = rib;
                        }
                    }
                }
                ...
                if (tap->IsHostedNetwork() && !exchangeof(route_added_, true)) {
                    ...
                    if (auto tun_ni = tun_ni_; NULL != tun_ni) {
            ...
            void VEthernetNetworkSwitcher::AddRouteWithDnsServers() noexcept {
                ...
                do {
                    std::shared_ptr<NetworkInterface> nis[] = { tun_ni_, underlying_ni_ };
            ...
            void VEthernetNetworkSwitcher::ReleaseAllObjects(bool ctor) noexcept {
                // Clear event bindings, including the ICMP packet container.
                icmppackets_.clear();
                TickEvent.reset();
                ...
                if (auto protector = std::move(protect_network_); NULL != protector) {
                    ...
                    // In android platform you need to request the ExitJNI function of the network protector.
#if defined(_ANDROID)
                    protector->ExitJNI();
#endif
                }
