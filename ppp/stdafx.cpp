...
namespace ppp {
    ...
    bool HasCommandArgument(const char* name, int argc, const char** argv) noexcept {
        ...
        auto f = [](ppp::string& commandText, const ppp::string& name) noexcept -> bool {
        ...
        bool result = false;
        result = result || f(commandText, name + ppp::string("="));
        result = result || f(commandText, name + ppp::string(" "));
        return result;
    }
    ...
    void PrintToConsoleForegroundWindow(std::size_t padding_length, char padding_char, const char* format, ...) noexcept {
        va_list ap;
        va_start(ap, format);
        PrintToConsoleForegroundWindow(padding_length, padding_char, format, ap);
        va_end(ap);
    }

    void PrintToConsoleForegroundWindow(std::size_t padding_length, char padding_char, const char* format, va_list args) noexcept {
        ppp::string result = PrintToString(padding_length, padding_char, format, args);
        fprintf(stdout, "%s", result.data());
    }

    ppp::string PrintToString(std::size_t padding_length, char padding_char, const char* format, va_list args) noexcept {
        int length = 0;
        std::shared_ptr<char> buffer = make_shared_alloc<char>(PPP_BUFFER_SIZE);
        if (NULL != buffer) {
            length = vsnprintf(buffer.get(), PPP_BUFFER_SIZE, format, args);
        }

        ppp::string result;
        if (NULL != buffer && length > 0) {
            result = ppp::string(buffer.get(), length);
        }

        return PaddingRight(result, padding_length, padding_char);
    }

    ppp::string PrintToString(std::size_t padding_length, char padding_char, const char* format, ...) noexcept {
        va_list ap;
        va_start(ap, format);
        ppp::string result = PrintToString(padding_length, padding_char, format, ap);
        va_end(ap);
        return result;
    }
